...
public class SecureController extends InitiatedController {
	...
	public String profile(Model model) {
		Log.debug(
				"***************************************** SecureController.profile() *****************************************");

		Utilisateur user = userService.findById(
	...
	@PostMapping("/profile")
	public String saveAuthorName(@Valid @ModelAttribute("author") String authorName, BindingResult br, Model model) {

		Log.debug(
				"***************************************** SecureController.saveAuthorName() *****************************************");
		if (br.hasErrors()) {
			// return goEdit(user, model);
			Log.debug("SecureController.saveAuthorName() - br.hasErrors()");
		}
		...
		Auteur author = new Auteur(authorName, user.getPrenom(), user.getNom(), user);
		authorService.save(author);
		return "redirect:/index?authorcreated=true";
	}

	@PostMapping("/savequote")
	public String saveQuote(@Valid @ModelAttribute("quote") Quote quote, BindingResult br, Model model) {
		Log.debug(
				"***************************************** SecureController.saveQuote() *****************************************");
		if (br.hasErrors()) {
			// return goEdit(user, model);
